...
class SvmModel:
    ...
    def __init__(self):
        ...
        self.scaler = joblib.load("svm_scaler.pkl")
        background = joblib.load("shap_background_scaled.pkl")
        # LinearExplainer works for linear-kernel SVC (model has coef_ + intercept_)
        # Falls back to coef-based attribution for any linear SVM
        try:
            ...
            self._shap_mode = "linear"
        except Exception:
            # Fallback: manual linear attribution via model coefficients
            self._coef = self.model.coef_[0]
            self._bg_mean = np.mean(background, axis=0)
            self.base_value = float(np.mean(self.model.decision_function(background)))
            self._shap_mode = "manual"
    ...
    def predict(self, features: dict):
        ...
        if self._shap_mode == "linear":
            ...
            sv = self._coef * (scaled_input[0] - self._bg_mean)
